...
class lv_thermostat_card : lv.obj
  ...
  def init(parent)
    ...
    self.arc.set_align(lv.ALIGN_CENTER)
    self.arc.set_style_border_width(0, lv.PART_MAIN | lv.STATE_DEFAULT)
    self.arc.set_rotation(self.arc_angle / 2)
    self.arc.set_bg_angles(0, self.arc_angle)
    self.arc.remove_style(nil, lv.PART_KNOB)

    self.arc.add_event_cb( / -> self._arc_changed(), lv.EVENT_VALUE_CHANGED, 0)

    # create difference arc
    self.diff_arc = lv.arc(self.arc)
    self.diff_arc.set_align(lv.ALIGN_CENTER)
    self.diff_arc.set_style_border_width(0, lv.PART_MAIN | lv.STATE_DEFAULT)
    ...
    self.diff_arc.set_style_arc_color(lv.color(lv.COLOR_GREEN), 0)   # should not be needed
    # its knob
    self.diff_arc.remove_style(nil, lv.PART_KNOB)
    #self.diff_arc.set_style_outline_color(lv.color(lv.COLOR_RED), lv.PART_KNOB)
    #self.diff_arc.set_style_bg_color(lv.color(lv.COLOR_GREEN), lv.PART_KNOB)   # should not be needed

    self.setpoint_knob = lv.obj(self.diff_arc)
    self.setpoint_knob.set_style_pad_all(0, 0)
    self.setpoint_knob.set_style_border_width(4, lv.PART_MAIN)
    self.setpoint_knob.set_style_margin_all(0, 0)
    self.setpoint_knob.set_style_margin_bottom(5, 0)
    self.setpoint_knob.set_local_style_prop(lv.STYLE_BORDER_COLOR, lv.COLOR_RED, lv.PART_MAIN)
    self.setpoint_knob.set_style_bg_color(lv.color(lv.COLOR_RED), lv.PART_MAIN)
    self.setpoint_knob.set_style_bg_opa(50, lv.PART_MAIN)
    self.setpoint_knob.set_align(lv.ALIGN_CENTER)    # ???

    self.measurement_knob = lv.obj(self.diff_arc)
    self.measurement_knob.set_style_pad_all(0, 0)
    self.measurement_knob.set_style_border_width(2, lv.PART_MAIN)
    self.measurement_knob.set_style_margin_all(0, 0)
    self.measurement_knob.set_local_style_prop(lv.STYLE_BORDER_COLOR, lv.COLOR_BLUE, lv.PART_MAIN)
    self.measurement_knob.set_style_bg_color(lv.color(lv.COLOR_BLUE), lv.PART_MAIN)
    self.measurement_knob.set_style_bg_opa(50, lv.PART_MAIN)
    self.measurement_knob.set_align(lv.ALIGN_CENTER)    # ???
  ...
  def update_diff_arc()
    ...
    #self.diff_arc.rotate_obj_to_angle(self.measurement_knob, 0)
  end
  ...
  def _size_changed(obj, evt)
    ...
    self.arc.set_size(w, h)
    self.diff_arc.set_size(w, h)
  end
